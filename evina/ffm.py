...
class Ffm:
    def __init__(self) -> None:
        ...
        subprocess.run("bash -c 'git commit -a -m Add changes && git push --all'")
        if self.dict == {}:
            sys.exit()
        else:
            self.work()
    ...
    def work(self):
        ...
        for key,value in self.dict['evina'].items():
            ...
            if 'douyin' in value.rtmp_url :
                ...
                if not os.path.exists(file):
                    os.makedirs(file)
            data = ' '.join(['bash -c "ffmpeg -t 19800 -i ',"'",value.rtmp_url,"'",'-c:a copy -c:v copy -preset ultrafast -f segment -segment_time 3600 -strftime 1 ',os.path.join(file,"%Y-%m-%d-%H-%M-%S.mp4").replace('\\','/'),'"'])
            threading.Thread(target=self.ffm,args=(data,file,ali_file,key,value,)).start()
